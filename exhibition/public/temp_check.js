        ...
        function showPersonCard(index, person) {
            ...
            if (window._isPartialAdd) {
                ...
                if (connectionCount === 0) {
                    popupCopy = "There's nothing to discover yet.";
                } else {
        ...
        // Force English typing only and auto-capitalize first letters
        ...
        function goToWizardStep(step) {
            ...
            if (stepEl) {
                if (targetDomId === 10 || targetDomId === '10') {
            ...
            const stepsWithFooter = ['0', '4', '4a', 'rec-1', 'conn-1', '5', '5a', '5b', '6a', '6b', '6c', 'rec-2', 'conn-2', '7a', '7b', 'rec-3', 'conn-3', '8a', '8b', 'rec-4', 'conn-4'];
            ...
            const progressMap = {
                '0': 1, '4': 1, 'rec-1': 1, 'conn-1': 1,
                '4a': 2, 'rec-2': 2, 'conn-2': 2,
                '5': 3, '5a': 3, '5b': 3, '6a': 3, '6b': 3, '6c': 3, 'rec-3': 3, 'conn-3': 3,
                '7a': 4, '7b': 4, '8a': 4, '8b': 4, 'rec-4': 4, 'conn-4': 4
            };
            ...
            if (progressContainer) {
                ...
                if (step !== 10 && progressMap[step] !== undefined) {
                    ...
                    for (let i = 1; i <= 4; i++) {
                        ...
                        if (i < activeItemNum) {
                            progItem.className = 'wizard-progress-item completed';
                        } else if (i === activeItemNum) {
                            progItem.className = 'wizard-progress-item active';
                        } else {
                            progItem.className = 'wizard-progress-item';
                        }
                    }
                } else {
        ...
        function setupAutocomplete(inputId, staticPrefix) {
            ...
            const PREDEFINED = {
                ...
                'military': [
                    ...
                    'Givati Soldier (Givaton)', 'Nahal Soldier (Nahalawi)', 'Kfir Soldier (Kfirnik)', 'Unit 8200', 'Unit 669',
                    'Golani', 'Givati', 'Nahal', 'Paratroopers (Tzanhanim)', 'Kfir', 'Armored Corps (Shiryon)', 'Artillery (Totchanim)', 'Combat Engineering (Handasa Kravit)',
                ...
                'education': [
                    ...
                    'Visual Communication', 'Computer Science', 'Law', 'Engineering', 'Psychology', 'Medicine'
                ],
            ...
            input.addEventListener('input', (e) => {
                ...
                if (PREDEFINED[prefix]) {
                    PREDEFINED[prefix].forEach(val => {
                ...
                // Inline Autofill Logic (Word by Word)
                if (!isDeleting && val.length > 0) {
                    const startsWithMatches = Object.keys(counts)
                        .filter(k => k.toLowerCase().startsWith(searchValForMatch.toLowerCase()))
                        .sort((a, b) => counts[b] - counts[a]);
                        
                    if (startsWithMatches.length > 0) {
                        const bestMatch = startsWithMatches[0];
        ...
        function calculateMatchScore(p1, p2) {
            ...
            p2Tags.forEach(t => {
                ...
                const normTag = cleanTag.toLowerCase().replace(/\s+/g, '');
                
                const match = p1Tags.find(p1t => {
                    const cleanP1 = p1t.replace(/^#/, '');
                    const normP1 = cleanP1.toLowerCase().replace(/\s+/g, '');
                    return normTag === normP1 || 
                           (normTag.length > 3 && normP1.includes(normTag)) || 
                           (normP1.length > 3 && normTag.includes(normP1));
                });
        ...
        function generateRecommendations() {
            let tags = (document.getElementById('f-tags') ? document.getElementById('f-tags').value : '').split(',').map(t => t.trim()).filter(Boolean);
        ...
        function openModal(mode, personData = null, defaultConnectionNodeId = null) {
            ...
            if (mode === 'add') {
                ...
                // We show all form sections at once for Edit mode
                ['wizard-step-4', 'wizard-step-4a', 'wizard-step-5', 'wizard-step-6', 'wizard-step-9'].forEach(id => {
                ...
                // Show global footer and configure for edit mode
                const globalFooter = document.getElementById('global-wizard-footer');
                if (globalFooter) {
                    globalFooter.style.display = 'flex';
                    const btnBack = document.getElementById('btn-wizard-global-back');
                    ...
                    const btnNext = document.getElementById('btn-wizard-global-next');
                    if (btnNext) {
                        btnNext.textContent = 'Update Network';
                        btnNext.onclick = () => { processStep9(); };
                    }
                }
            }
        }
        ...
        function addConnectionEntry(defaultTargetId = null, defaultType = 'friend', defaultStrength = 3) {
            ...
            div.style.padding = '30px'; // Increased padding
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (document.getElementById('f-highschool')) addField([document.getElementById('f-highschool').value], 'highschool', true);
            if (document.getElementById('f-university')) addField([document.getElementById('f-university').value], 'university', true);
            if (document.getElementById('f-degree')) addField([document.getElementById('f-degree').value], 'degree', true);
            if (document.getElementById('f-army-role')) addField([document.getElementById('f-army-role').value], 'armyrole', true);
            if (document.getElementById('f-army-base')) addField([document.getElementById('f-army-base').value], 'armybase', true);
            addField(getFieldValues('tw-f-other-value', 'f-other-value'), 'other', true);
            ...
            const originCity = getFieldValues('tw-f-origin-city', 'f-origin-city').join(', ');
            ...
            try {
                ...
                if (newCompleteness > oldCompleteness) {
                    ...
                    if (mode === 'edit') {
                        notif.innerHTML = `Feed Successful!<br><span style="font-size:15px; opacity:0.6; display:block; margin-top:12px; letter-spacing: 0.5px;">Network volume increased by ${percentIncrease.toFixed(1)}%</span>`;
                    }
                    ...
                    // Expand network visually - update positionCache!
                ...
                if ((mode === 'add' || mode === 'edit') && numStrands > 0) {
                    ...
                    if (mode === 'add') {
                        ...
                        setTimeout(() => {
                            ...
                            // Follow the node back to its spot and open the info panel
                            setTimeout(() => {
                                selectNode(targetIdx, true);
                            }, 100);
